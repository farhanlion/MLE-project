"""
Bronze Layer Processing - User Logs
Load user_logs CSV into Bronze layer parquet with partitioning.
"""
...
def initialize_spark(app_name: str = "bronze_user_logs", driver_memory: str = "8g") -> SparkSession:
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("BRONZE LAYER - USER LOGS")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Get paths
        user_logs_path = "/app/data/user_logs.csv"
        user_logs_v2_path = "/app/data/user_logs_v2.csv"
        bronze_path = "/app/datamart/bronze/user_logs"

        # Read CSV
        v1 = read_csv(spark, user_logs_path )
        v2 = read_csv(spark, user_logs_v2_path)
        df_new = v1.union(v2)
        
        # Add source tracking
        source_name = Path(user_logs_path).name
        ...
        logger.info("=" * 80)
        logger.info("USER LOGS PROCESSING COMPLETED")
        logger.info("=" * 80)
