...
# coding: utf-8
"""
Train Logistic Regression. All reading, date-windowing, splitting, and sampling is done here.
Calls preprocessing_before_fit.preprocess_features_for_lr for feature transforms only.
"""
...
def main(args):
    ...
    spark = start_spark(app_name="kkbox_lr_train")

    # read parquet (features and labels) and filter global window
    ...
    # select feature columns (hardcoded as before)
    feature_cols = [
    ...
    # PREPROCESSING (call shared module) -> only feature transforms
    X_train_lr, [X_val_lr, X_test_lr, X_oot_lr], scaler, lr_cols = preprocess_features_for_lr(
    ...
    with mlflow.start_run(run_name="LogisticRegression_tuning"):
        ...
        # evaluate and log
        for name, X, y in [("train", X_train_lr, y_train), ("val", X_val_lr, y_val),
                           ("test", X_test_lr, y_test), ("oot", X_oot_lr, y_oot)]:
