...
=======================================
Creates gold layer features.

Usage:
    # Process all data (default)
    python gold_feature_store_prod.py
    
    # Process single date
    python gold_feature_store_prod.py --date 2017-03-01
"""
...
def create_userlog_features(df_userlogs: DataFrame, snapshot_users: DataFrame) -> DataFrame:
    ...
    # Select and cast snapshot_date, removing any duplicates
    snapshot_users_clean = snapshot_users.select(
        "msno",
        F.to_date("snapshot_date").alias("snapshot_date")
    ).distinct()
    
    # Cache snapshot users for multiple joins
    snapshot_users_cached = snapshot_users_clean.cache()
...
def create_transaction_features(
    ...
    # Ensure date types and select only needed columns to avoid ambiguity
    df_transactions = df_transactions.withColumn("transaction_date", F.to_date("transaction_date"))
    
    # Clean registered_users to avoid ambiguous columns
    registered_users_clean = registered_users.select(
        "msno",
        F.to_date("snapshot_date").alias("snapshot_date"),
    ...
    r = registered_users_clean.alias("r")
    ...
    tenure_asof = latest_tx.join(
        registered_users_clean.select("msno", "snapshot_date", "registration_date"),
        ["msno", "snapshot_date"],
        "left"
    ).withColumn(
        "tenure_days",
...
def process_features(
    ...
    # Create snapshot users - ensure no duplicate columns
    snapshot_users = txn_snapshots.select("snapshot_date", "msno").distinct()
    
    # Join with members, being explicit about columns to avoid duplicates
    members_cols = [col for col in tables['members'].columns if col != 'snapshot_date']
    snapshot_users = snapshot_users.join(
        tables['members'].select("msno", *[c for c in members_cols if c != 'msno']),
        on="msno",
        how="left"
    )
